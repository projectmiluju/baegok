...
- **Issue #3 — GitHub OAuth + JWT 세션 구현 (진행 중, QA 승인)**
  ...
  - state CSRF 보호, Hono 미들웨어, 28개 자동화 테스트 (보안 엣지 17 포함)
...
| 이슈                                     | 심각도 | 상태                                                    |
| ---------------------------------------- | ------ | ------------------------------------------------------- |
| **`docker-compose.yml` 실제 부재**       | P1     | #1 결손 — 별도 fix 이슈로 분리 예정 (#3 머지 직후 진행) |
...
| 항목                                                             | 등록일     | 예상 작업량    |
| ---------------------------------------------------------------- | ---------- | -------------- |
| `docker-compose.yml` 작성 (Postgres + Redis + Kafka + Zookeeper) | 2026-04-10 | S              |
...
- [ ] `docker-compose.yml` 작성 (#1 결손 fix)
